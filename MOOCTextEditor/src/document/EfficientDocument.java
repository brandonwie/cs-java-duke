...
public class EfficientDocument extends Document {
	...
	private boolean isWord(String tok) {
		...
		return !(tok.indexOf("!") >= 0 || tok.indexOf(".") >= 0
				|| tok.indexOf("?") >= 0);
	}
	...
	private void processText() {
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (isWord(token)) {
				numOfWords++;
				numOfSyllables += countSyllables(token);
			} else {
				numOfSentences++;
			}
			// if last word or single
			if (i == tokens.size() - 1 && isWord(token)) {
				numOfSentences++;
			}
		}
		numWords = numOfWords;
		numSentences = numOfSentences;
		numSyllables = numOfSyllables;
	}
	...
	public static void main(String[] args) {
		...
		testCase(
				new EfficientDocument("sentence, with, lots, of, commas.!  "
						+ "(And some poaren)).  The output is: 7.5."),
				15, 11, 4);
		testCase(new EfficientDocument("many???  Senteeeeeeeeeences are"), 6, 3,
				2);
