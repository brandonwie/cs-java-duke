...
public class NearbyWords implements SpellingSuggest {
	...
	public void substitution(String s, List<String> currentList,
		...
		for (int index = 0; index < s.length(); index++) {
			for (int charCode = (int) 'a'; charCode <= (int) 'z'; charCode++) {
				...
				// change char for each letter (s1 a, s1 b, s1 c, ...)
	...
	public void insertions(String s, List<String> currentList,
		...
		for (int i = 0; i <= s.length(); i++) {
			for (int charCode = (int) 'a'; charCode <= (int) 'z'; charCode++) {
				StringBuffer sb = new StringBuffer(s);
				// if index is the same as s.length()
				if (i == s.length()) {
					// append chars
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		// initial variables
		LinkedList<String> queue = new LinkedList<String>(); // String to
																// explore
	...
	public static void main(String[] args) {
		...
		System.out
				.println("One away word Strings for for \"" + word + "\" are:");
