 ...
 * team
 * 
 * @author Seokhyun Wie Date: November, 07, 2020
 */
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() {
		...
		int countOcean = 0;
		for (Marker cm : countryMarkers) {
			String cmCountry = (String) cm.getProperty("name");
			int countLand = 0;

			for (Marker m : quakeMarkers) {
				// assign m to EarthquakeMarker to use `.isOnLand`
				EarthquakeMarker em = (EarthquakeMarker) m;
				boolean isOnLand = em.isOnLand();
				// if quakeMarker is on land,
				if (isOnLand) {
					// assign the Marker m to LandQuakeMarker if m.onLand() is
					// true, to use `.getCountry()` method
					LandQuakeMarker lm = (LandQuakeMarker) m;
					String mCountry = (String) lm.getCountry();
					if (cmCountry == mCountry) {
						countLand++;
					}
				} else {
					countOcean++;
			...
			if (countLand > 0) {
				System.out.println(
						cmCountry + " had " + countLand + " earthquakes");
			}
		}
		// Ocean quakes
		System.out.println(countOcean / countryMarkers.size());
		...
		//
		// * To get the name of a country from a country marker in variable cm,
		// use:
