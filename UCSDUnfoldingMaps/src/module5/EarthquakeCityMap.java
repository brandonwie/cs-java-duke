...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseMoved() {
		...
		selectMarkerOnHover(quakeMarkers);
		selectMarkerOnHover(cityMarkers);
	}
	...
	private void checkCitiesOnClick() {
		// an earthquake has already been found as clicked
		if (lastClicked != null)
			return;
		...
		for (Marker marker : cityMarkers) {
			if (!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
				...
				for (Marker mhide : quakeMarkers) {
					...
					if (quakeMarker.getDistanceTo(marker
							.getLocation()) > quakeMarker.threatCircle()) {
	...
	// and respond appropriately
	private void checkEarthquakesOnClick() {
		if (lastClicked != null)
			return;
		// Loop over the earthquake markers to see if one of them is selected
		...
		for (Marker country : countryMarkers) {
			...
			for (Marker marker : quakeMarkers) {
				...
				if (eqMarker.isOnLand()) {
					if (countryName
							.equals(eqMarker.getStringProperty("country"))) {
						numQuakes++;
					}
				}
			}
